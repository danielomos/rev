package com.revomvpandriodapp.app.modules.homeemailtoken.`data`.viewmodel
...
import com.revomvpandriodapp.app.modules.homeemailtoken.`data`.model.HomeEmailtokenModel
...
class HomeEmailtokenVM : ViewModel(), KoinComponent {
  val homeEmailtokenModel: MutableLiveData<HomeEmailtokenModel> =
      MutableLiveData(HomeEmailtokenModel())
  ...
  fun callCreateMobileTokenVerificationApi() {
    viewModelScope.launch {
      ...
      contentType = """application/json""",
          ...
          token = homeEmailtokenModel.value?.etHomeEmailtokenValue)
      )
      )
  ...
  fun bindCreateMobileTokenVerificationResponse(response: CreateMobileTokenVerificationResponse) {
    val homeEmailtokenModelValue = homeEmailtokenModel.value ?:HomeEmailtokenModel()
    prefs.setMessage(response.payload?.message)
    homeEmailtokenModel.value = homeEmailtokenModelValue
  }
}
