package com.revomvpandriodapp.app.modules.homeemailtoken.ui
...
import com.revomvpandriodapp.app.databinding.ActivityHomeEmailtokenBinding
...
import com.revomvpandriodapp.app.modules.homeemailtoken.`data`.viewmodel.HomeEmailtokenVM
...
class HomeEmailtokenActivity :
    BaseActivity<ActivityHomeEmailtokenBinding>(R.layout.activity_home_emailtoken) {
  private val viewModel: HomeEmailtokenVM by viewModels<HomeEmailtokenVM>()

  override fun onInitialized(): Unit {
    viewModel.navArguments = intent.extras?.getBundle("bundle")
    binding.homeEmailtokenVM = viewModel
  }
  ...
  override fun setUpClicks(): Unit {
    ...
    binding.btnVerifyEmailAddress.setOnClickListener {
      this@HomeEmailtokenActivity.hideKeyboard()
  ...
  override fun addObservers(): Unit {
    ...
    viewModel.progressLiveData.observe(this@HomeEmailtokenActivity) {
      if(it) {
        progressDialog?.dismiss()
        progressDialog = null
        progressDialog = this@HomeEmailtokenActivity.showProgressDialog()
      } else {
        progressDialog?.dismiss()
      }
    }
    viewModel.createMobileTokenVerificationLiveData.observe(this@HomeEmailtokenActivity) {
  ...
  private fun onErrorCreateMobileTokenVerification(exception: Exception): Unit {
    when(exception) {
      ...
      is HttpException -> {
        ...
        this@HomeEmailtokenActivity.alert(MyApp.getInstance().getString(R.string.lbl_error),errMessage) {
          neutralButton {
          }
        }
      }
  ...
  companion object {
    const val TAG: String = "HOME_EMAILTOKEN_ACTIVITY"


    fun getIntent(context: Context, bundle: Bundle?): Intent {
      val destIntent = Intent(context, HomeEmailtokenActivity::class.java)
